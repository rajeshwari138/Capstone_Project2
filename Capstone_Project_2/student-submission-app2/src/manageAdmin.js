...
import {Card, Typography, Grid, Button, CardActions, CardContent, Avatar,
   CssBaseline, TextField, FormControlLabel, Checkbox, Container, Box, Divider} from '@material-ui/core';
...
export default function ManageAdmin() {
    ...
    if(role !== "Admin"){
      ...
      useEffect( async ()=> {
        ...
        if(decoded.exp*1000 <= Date.now()) {
            ...
            try{
            var response = await axios.get('http://localhost:3000/admin/getadmin');
    ...
    const updateAdmin = async(id) => {
        ...
        if(decoded.exp*1000 <= Date.now()) {
            ...
            try{
                await axios.delete(`https://student-task-server.herokuapp.com/admin/${id}`);
    ...
    const handleSubmit = async (event) => {
        ...
        if(decoded.exp*1000 <= Date.now()) {
            ...
            // eslint-disable-next-line no-console
              ...
              var response = await axios.post('https://student-task-server.herokuapp.com/admin/register', {
              ...
              if(response.data) {
                  setAdmin(admin.concat(response.data));
                  }
              } catch(err) {
